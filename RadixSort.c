...
void radixSort(Jogador *lidos[], int n, int trocas) {
  int max_id = lidos[0]->id;
  for (int i = 1; i < n; i++) {
    if (lidos[i]->id > max_id) {
      max_id = lidos[i]->id;   //Determina o id Máximo.
    }
  }

  for (int exp = 1; max_id / exp > 0; exp *= 10) {
    int output[n]; //Loop que passa por cada dígito.
    int count[10] = {0};

    for (int i = 0; i < n; i++) {
      count[(lidos[i]->id / exp) % 10]++;
    } //Conta o número de ocorrências de cada dígito na posição atual
    ...
    for (int i = n - 1; i >= 0; i--) {
      output[count[(lidos[i]->id / exp) % 10] - 1] = lidos[i]->id;
      count[(lidos[i]->id / exp) % 10]--;
    } //É construído um array baseado na ordem correta dos dígitos.

    for (int i = 0; i < n; i++) {
      lidos[i]->id = output[i]; //É copiado para o arrau principal
    }
  }
}
...
void criarLog(int trocas,double tempoExecucao) {
    FILE *arquivo;
    arquivo = fopen("801516_RadixSort.txt", "w"); // Abre o arquivo para escrita (cria se não existir
